    ...
    return (
        <div className="flex flex-col h-full bg-white dark:bg-gray-900/50 rounded-xl border border-gray-200 dark:border-gray-800 overflow-hidden transition-colors">
            <div className="p-3 md:p-4 border-b border-gray-200 dark:border-gray-800 bg-gray-50 dark:bg-gray-800/30 flex justify-between items-center">
                ...
                <button onClick={fetchLogs} className="text-[9px] md:text-[10px] text-blue-400 hover:text-blue-300 font-bold uppercase">Refresh</button>
            </div>
            ...
            <div className="flex-1 overflow-y-auto">
                ...
                <div className="md:hidden divide-y divide-gray-800">
                    {loading ? (
                        <div className="p-8 text-center text-gray-500 italic animate-pulse text-xs">Synchronizing audio data...</div>
                    ) : logs.length === 0 ? (
                        <div className="p-8 text-center text-gray-500 italic text-xs">No audio logs captured.</div>
                    ) : logs.map(log => (
                        <div key={log.Id} className="p-4 space-y-3">
                            <div className="flex justify-between items-center">
                                ...
                                <div className="flex items-center gap-2">
                                    ...
                                    <button className="p-2 bg-blue-600/20 text-blue-400 rounded-lg border border-blue-500/30">
                                        <Play size={12} />
                                    </button>
                                </div>
                            </div>
                            <div className="bg-black/30 p-3 rounded-lg border border-gray-800 shadow-inner">
                                <p className="text-sm text-gray-200 italic leading-relaxed">"{log.TranscribedText}"</p>
                            </div>
                            {JSON.parse(log.FlaggedKeywordsJson || "[]").length > 0 && (
                                <div className="flex flex-wrap gap-1.5 pt-1">
                                    {JSON.parse(log.FlaggedKeywordsJson || "[]").map((k: string) => (
                                        <span key={k} className="text-[9px] bg-red-500/10 text-red-500 border border-red-500/20 px-2 py-0.5 rounded font-black uppercase">
                                            {k}
                                        </span>
                                    ))}
                                </div>
