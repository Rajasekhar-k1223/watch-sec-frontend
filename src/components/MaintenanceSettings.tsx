...
import { Calendar, Clock, Save, Check, AlertCircle, Zap } from 'lucide-react';
        ...
        <div className="space-y-6 animate-fade-in">
            ...
            {config.enabled && (
                <div className="space-y-6 p-6 bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm animate-fade-in">
                    ...
                    <div>
                        <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-3">Update Strategy</label>
                        <div className="flex gap-2 p-1 bg-gray-100 dark:bg-gray-900 rounded-lg w-fit">
                            <button
                                onClick={() => setConfig({ ...config, mode: 'automatic' })}
                                className={`px-4 py-2 rounded-md text-sm font-bold transition-all ${config.mode === 'automatic'
                            ...
                            <button
                                ...
                                className={`px-4 py-2 rounded-md text-sm font-bold transition-all ${config.mode === 'manual'
                                        ? 'bg-blue-600 text-white shadow-md'
                                        : 'text-gray-500 hover:text-gray-700 dark:hover:text-gray-300'
                                    }`}
                            >
                        ...
                        <p className="mt-2 text-xs text-gray-400">
                            {config.mode === 'automatic' 
                                ? 'Agents update automatically during the selected window.' 
                                : 'Agents only update on specifically scheduled dates below.'}
                        </p>
                        <div className="mt-3 p-3 bg-blue-50 dark:bg-blue-500/10 rounded-lg border border-blue-100 dark:border-blue-500/20 flex items-start gap-3">
                            <Zap size={14} className="text-blue-500 mt-0.5" />
                            <p className="text-[10px] text-blue-700 dark:text-blue-300 font-medium">
                                <span className="font-bold">Pro Tip:</span> You can always use the <span className="italic font-bold">"Patch Now"</span> button in the Agent Details modal to force an immediate update, bypassing all maintenance windows.
                            </p>
                        </div>
                    </div>
                    ...
                    <div className="pt-4 border-t border-gray-100 dark:border-gray-700">
                        <div className="flex items-center justify-between mb-4">
                            <div>
                                <label className="block text-sm font-medium text-gray-900 dark:text-white">One-time Maintenance Date</label>
                                <p className="text-xs text-gray-400">Schedules a single manual patching event.</p>
                            </div>
                            <button
                                onClick={() => setConfig({ ...config, oneTimeEnabled: !config.oneTimeEnabled })}
                                className={`w-12 h-6 rounded-full p-1 transition-colors flex items-center ${config.oneTimeEnabled ? 'bg-green-500' : 'bg-gray-200 dark:bg-gray-600'}`}
                            >
                                <div className={`w-4 h-4 bg-white rounded-full shadow-md transform transition-transform ${config.oneTimeEnabled ? 'translate-x-6' : 'translate-x-0'}`} />
                            </button>
                        </div>
                        
                        {config.oneTimeEnabled && (
                            <div className="animate-fade-in">
                                <input
                                    type="date"
                                    value={config.oneTimeDate || ''}
                                    onChange={(e) => setConfig({ ...config, oneTimeDate: e.target.value })}
                                    className="w-full bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-lg px-4 py-2 outline-none focus:border-blue-500 text-gray-900 dark:text-white"
                                />
                            </div>
                        )}
