    ...
    useEffect(() => {
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 h-[calc(100vh-100px)]">
            ...
            <div className="lg:col-span-2 flex flex-col gap-6">
                {/* Stream / Connection Area */}
                <div className="bg-gray-800 rounded-xl overflow-hidden border border-gray-700 shadow-lg relative flex-1 min-h-[400px]">
                    {/* ... existing stream code ... */}
                    {/* Placeholder for brevity, assuming existing stream code is here */}
                    <div className="absolute inset-0 flex items-center justify-center bg-gray-900">
                        <p className="text-gray-500">Video Stream Component Here</p>
                    </div>
                </div>

                {/* Console / Terminal (Optional) */}
                <div className="bg-gray-800 rounded-xl border border-gray-700 p-4 h-48 overflow-y-auto font-mono text-xs text-green-400">
                    <h3 className="text-white font-bold mb-2 sticky top-0 bg-gray-800">Agent Logs</h3>
                    {/* ... logs ... */}
                </div>
            </div>

            {/* RIGHT COLUMN: SECURITY EVENTS */}
            <div className="bg-gray-800 rounded-xl border border-gray-700 flex flex-col shadow-lg overflow-hidden">
                <div className="p-4 border-b border-gray-700 bg-gray-900/50 flex justify-between items-center">
                    <h3 className="text-white font-bold flex items-center gap-2">
                        <ShieldAlert size={18} className="text-red-500" />
                        Security Alerts
                    </h3>
                    <span className="text-xs bg-red-500/10 text-red-500 px-2 py-1 rounded-full animate-pulse">Live</span>
                </div>

                <div className="flex-1 overflow-y-auto p-4 space-y-3">
                    {(events[selectedAgentId] || []).length === 0 && (
                        <div className="text-center text-gray-500 mt-10 text-sm">No recent alerts</div>
                    )}

                    {(events[selectedAgentId] || []).map((evt, i) => (
                        <div key={i} className="bg-gray-900/40 border border-gray-700 rounded p-3 text-sm hover:border-gray-500 transition-colors">
                            <div className="flex justify-between mb-1">
                                <span className={`font-bold ${evt.type?.includes('USB') ? 'text-yellow-400' :
                                        evt.type?.includes('FILE') ? 'text-blue-400' :
                                            evt.type?.includes('NETWORK') ? 'text-orange-400' : 'text-white'
                                    }`}>
                                    {evt.type || 'UNKNOWN_EVENT'}
                                </span>
                                <span className="text-gray-500 text-xs">{new Date(evt.timestamp).toLocaleTimeString()}</span>
                            </div>
                            <p className="text-gray-300 break-words">{evt.details}</p>
                        </div>
                    ))}
                </div>
            </div>
